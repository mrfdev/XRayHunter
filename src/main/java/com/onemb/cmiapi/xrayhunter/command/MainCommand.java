...
import com.onemb.cmiapi.xrayhunter.BuildInfo;
import com.onemb.cmiapi.xrayhunter.XRayHunterFeature;
import com.onemb.cmiapi.xrayhunter.XRayHunterSettings;
...
public class MainCommand implements CommandExecutor, TabCompleter {
    ...
    public MainCommand(XRayHunterFeature feature) {
        this.feature = feature;
        this.lookupCommand = new LookupCommand(feature);
        this.detailCommand = new DetailCommand(feature);
        this.teleportCommand = new TeleportCommand(feature);
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
        if (!feature.isActive()) {
            sender.sendMessage("§cThe XRayHunter feature is currently unavailable.");
            return true;
        }
    ...
    public @Nullable List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length >= 2 && isSubcommand(args[0], "debug")) {
            ...
            if (args.length == 3 && isSubcommand(args[1], "set")) {
                return filterSuggestions(XRayHunterSettings.logicalConfigKeys(), args[2]);
            }
            ...
            if (args.length == 3 && isSubcommand(args[1], "whitelist")) {
                ...
                suggestions.addAll(feature.getSettings().excludedPlayers());
            ...
            if (args.length == 4 && isSubcommand(args[1], "whitelist", "wl")) {
                if (isSubcommand(args[2], "remove", "delete", "del")) {
                    return filterSuggestions(feature.getSettings().excludedPlayers(), args[3]);
                }
                return List.of();
            }
            return List.of();
    ...
    private boolean sendHelp(CommandSender sender) {
        final BuildInfo buildInfo = feature.getBuildInfo();
        ...
        sender.sendMessage("§f/xrayhunter lookup [time|alltime] [world|allworlds] [-all] §7- Query CoreProtect block breaks in one world or across all worlds, including database-only worlds.");
        ...
        sender.sendMessage("§7Default lookup time: §f" + feature.getSettings().defaultLookupTime());
        ...
        sender.sendMessage("§7Console implicit all-world limit: §f" + feature.getSettings().consoleMaxAllWorldLookupTime());
        sender.sendMessage("§7Explicit archive lookup: §f/xrayhunter lookup alltime allworlds");
        sender.sendMessage("§7Console compact view: §f" + (feature.getSettings().consoleHighValueOnly() ? "high-value only §7(use §f-all§7 to expand)" : "all tracked columns"));
    ...
    private boolean handleReloadCommand(CommandSender sender) {
        if (!hasAdminPermission(sender)) {
            sender.sendMessage("§cYou do not have permission to reload this feature.");
            return true;
        }

        feature.reloadConfiguration();
        sender.sendMessage("§aReloaded §f" + feature.getConfigurationFile().getPath() + "§a.");
        return true;
    }
    ...
    private void sendDebugOverview(CommandSender sender) {
        final BuildInfo buildInfo = feature.getBuildInfo();
        final Plugin coreProtectPlugin = feature.getCoreProtectPlugin();
        final File configFile = feature.getConfigurationFile();
        final @Nullable File databaseFile = feature.getCoreProtectDatabaseFile();
        ...
        sender.sendMessage(cleanMetaLine("Server", feature.getServer().getVersion()));
        sender.sendMessage(cleanMetaLine("Data folder", feature.getDataFolder().getAbsolutePath()));
        sender.sendMessage(cleanMetaLine("Config file", configFile.getAbsolutePath()));
        sender.sendMessage(cleanMetaLine("CoreProtect hooked", Boolean.toString(feature.isCoreProtectHooked())));
        ...
        if (coreProtectPlugin != null) {
            ...
            sender.sendMessage(cleanMetaLine("CoreProtect API", feature.getCoreProtectApi() == null
                    ? "unavailable"
                    : Integer.toString(feature.getCoreProtectApi().APIVersion())));
        ...
        sender.sendMessage(cleanMetaLine(
                "Latest tracked block data",
                formatTrackedTimestamp(feature.getMiningHistory().getLatestTrackedTimeSnapshot())
        ));
        sender.sendMessage(cleanMetaLine("Console high-value-only", Boolean.toString(feature.getSettings().consoleHighValueOnly())));
        sender.sendMessage(cleanMetaLine("Excluded players", feature.getSettings().excludedPlayers().isEmpty() ? "none" : Integer.toString(feature.getSettings().excludedPlayers().size())));
        sender.sendMessage(cleanMetaLine("Known CoreProtect worlds", Integer.toString(lookupCommand.getWorldNameSuggestions().size())));
        sender.sendMessage(cleanMetaLine("Cached investigation sessions", Integer.toString(feature.getSessionCount())));
        sender.sendMessage(cleanMetaLine("Summary cache entries", Integer.toString(feature.getMiningHistory().getSummaryCacheSize())));
        sender.sendMessage(cleanMetaLine("Summary cache hits", Long.toString(feature.getMiningHistory().getSummaryCacheHitCount())));
        sender.sendMessage(cleanMetaLine("Summary cache misses", Long.toString(feature.getMiningHistory().getSummaryCacheMissCount())));
        sender.sendMessage(cleanMetaLine("Default lookup time", feature.getSettings().defaultLookupTime()));
        sender.sendMessage(cleanMetaLine("Top results", Integer.toString(feature.getSettings().topResults())));
        sender.sendMessage(cleanMetaLine("Detail page size", Integer.toString(feature.getSettings().detailPageSize())));
    }
    ...
    private void sendDebugCommands(CommandSender sender) {
        ...
        sender.sendMessage("§f/xrayhunter lookup alltime allworlds §7(console = explicit full archive lookup)");
        sender.sendMessage("§f/xrayhunter lookup 365d allworlds §7(console = explicit large archive lookup)");
    ...
    private void sendDebugConfig(CommandSender sender) {
        ...
        sender.sendMessage(cleanMetaLine("startup.self-check-enabled", Boolean.toString(feature.getSettings().startupSelfCheckEnabled())));
        sender.sendMessage(cleanMetaLine("defaults.lookup-time", feature.getSettings().defaultLookupTime()));
        sender.sendMessage(cleanMetaLine("display.top-results", Integer.toString(feature.getSettings().topResults())));
        sender.sendMessage(cleanMetaLine("display.detail-page-size", Integer.toString(feature.getSettings().detailPageSize())));
        sender.sendMessage(cleanMetaLine("console.allow-server-wide-lookups", Boolean.toString(feature.getSettings().consoleAllowServerWideLookups())));
        sender.sendMessage(cleanMetaLine("console.high-value-only", Boolean.toString(feature.getSettings().consoleHighValueOnly())));
        sender.sendMessage(cleanMetaLine("console.max-all-world-lookup-time", feature.getSettings().consoleMaxAllWorldLookupTime()));
        sender.sendMessage(cleanMetaLine(
                "console.high-value-display-materials",
                joinMaterials(feature.getSettings().consoleHighValueDisplayMaterials())
        ));
        sender.sendMessage(cleanMetaLine(
                "filters.excluded-players",
                joinValues(feature.getSettings().excludedPlayers())
        ));
        sender.sendMessage(cleanMetaLine(
                "tracking.overworld.lookup-materials",
                joinMaterials(feature.getSettings().overworldLookupMaterials())
        ));
        sender.sendMessage(cleanMetaLine(
                "tracking.overworld.display-materials",
                joinMaterials(feature.getSettings().overworldDisplayMaterials())
        ));
        sender.sendMessage(cleanMetaLine(
                "tracking.nether.lookup-materials",
                joinMaterials(feature.getSettings().netherLookupMaterials())
        ));
        sender.sendMessage(cleanMetaLine(
                "tracking.nether.display-materials",
                joinMaterials(feature.getSettings().netherDisplayMaterials())
        ));
        sender.sendMessage(cleanMetaLine(
    ...
    private boolean handleDebugSet(CommandSender sender, String[] args) {
        if (args.length < 4) {
            ...
            sender.sendMessage("§7Supported keys: §f" + String.join("§7, §f", XRayHunterSettings.logicalConfigKeys()));
            return true;
        }
        ...
        feature.getConfiguration().set(key, parsedValue);
    ...
    private boolean addWhitelistEntry(CommandSender sender, String rawPlayerName) {
        ...
        final List<String> excludedPlayers = new ArrayList<>(feature.getSettings().excludedPlayers());
    ...
    private void sendWhitelistEntries(CommandSender sender) {
        final List<String> excludedPlayers = feature.getSettings().excludedPlayers();
    ...
    private void saveExcludedPlayers(List<String> excludedPlayers) {
        feature.getConfiguration().set("filters.excluded-players", excludedPlayers);
        feature.saveConfiguration();
        feature.reloadConfiguration();
    }
