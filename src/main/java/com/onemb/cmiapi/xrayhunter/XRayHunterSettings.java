 ...
 */
public record XRayHunterSettings(
    ...
    public static XRayHunterSettings load(XRayHunterFeature feature, FileConfiguration configuration) {
        ...
        return new XRayHunterSettings(
                ...
                parseMaterialList(feature, configuration, "console.high-value-display-materials", DEFAULT_CONSOLE_HIGH_VALUE_DISPLAY_MATERIALS),
                parsePlayerList(configuration, "filters.excluded-players"),
                parseMaterialList(feature, configuration, "tracking.overworld.lookup-materials", DEFAULT_OVERWORLD_LOOKUP_MATERIALS),
                parseMaterialList(feature, configuration, "tracking.overworld.display-materials", DEFAULT_OVERWORLD_DISPLAY_MATERIALS),
                parseMaterialList(feature, configuration, "tracking.nether.lookup-materials", DEFAULT_NETHER_LOOKUP_MATERIALS),
                parseMaterialList(feature, configuration, "tracking.nether.display-materials", DEFAULT_NETHER_DISPLAY_MATERIALS)
        );
    }
    ...
    private static List<Material> parseMaterialList(
        ...
        for (String configuredName : configuredNames) {
            ...
            if (material == null) {
                feature.getLogger().warning("Ignoring unknown material in " + path + ": " + configuredName);
                continue;
            }
        ...
        if (materials.isEmpty()) {
            feature.getLogger().warning("No valid materials found in " + path + ", using defaults instead.");
            return defaults;
        }
